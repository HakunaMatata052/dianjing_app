            ...
            <div class="video-list">
              <div class="video-item" v-for="(item,index) in videoList" :key="index">
                ...
                <div class="info">
                  <h3>{{item.comment}}</h3>
                  <div class="user">
                    <van-image fit="cover" :src="item.frontcover" class="avatar" @click.stop="$router.push(`/accompanyDetail/${item.userid}`)"/>
                    <div class="user-info">
                      <div class="username">
                        {{item.nickname}}
                  ...
                  <operation class="operation" :id="item.id" :comment="item.replyCount" :zan="item.likeCount" :iszan="true" />
                </div>
              </div>
            </div>
          </van-list>
